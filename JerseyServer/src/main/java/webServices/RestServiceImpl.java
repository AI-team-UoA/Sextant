...
public class RestServiceImpl {
	...
	public void init() {
		...
		// Layers' URLs format
		// HTTP + servlet.getServerName() + ":" + servlet.getServerPort() +
		// context.getContextPath()

		// System.out.println("********* servlet.getServerName():
		// "+servlet.getServerName());
		// System.out.println("********* servlet.getServerPort():
		// "+servlet.getServerPort());

		// System.out.println("********* servlet.getRemoteHost():
		// "+servlet.getRemoteHost());
		// System.out.println("********* servlet.getRemotePort():
		// "+servlet.getRemotePort());
	...
	public String saveMap(String mapInfo,
		...
		// Create new mapID or keep the existing one. Also add creation/modification
		// date
		String mapID = mapId;
		String createDate;
		String modifyDate;
		...
		if (chartsInfo != null) {
			...
			for (int i = 0; i < charts.size(); i++) {
				memoryStore.saveChart(i, charts.get(i).getQuery(), charts.get(i).getEndpoint(), charts.get(i).getType(),
						charts.get(i).getMeasures(), charts.get(i).getFreeDims(), charts.get(i).getInstances());
			}
		}
	...
	public List<MapInfo> getMapLayers(@PathParam("id") String id, @PathParam("host") String host,
		...
		if (chartPosition != -1) {
			for (int i = chartPosition; i < results.size(); i++) {
				...
				MapInfo info = new MapInfo("chart", temp.get(2), temp.get(3), temp.get(1), format, temp.get(4), temp.get(5),
						temp.get(6));
				mapInformation.add(info);
			}
		}
	...
	public String getMapInformation(@PathParam("id") String id, @PathParam("host") String host,
		...
		if (PROXY_STATUS.equalsIgnoreCase("on")) {
			...
			format = format.concat(HTTP + servlet.getServerName() + ":" + servlet.getServerPort() + context.getContextPath()
					+ "/" + STORE_FOLDER);
		}
		format = format.concat("$");
	...
	public String loadFile(@FormDataParam("uploadfile") InputStream fileInputStream,
		...
		try {
			...
			if (PROXY_STATUS.equalsIgnoreCase("on")) {
				fileURI = HTTP + PROXY_NAME + ":" + PROXY_PORT + context.getContextPath() + "/" + STORE_FOLDER + name
						+ fileType;
			} else {
				fileURI = HTTP + servlet.getServerName() + ":" + servlet.getServerPort() + context.getContextPath() + "/"
						+ STORE_FOLDER + name + fileType;
			}
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
	...
	public String startDownload(String aurl) {
		...
		try {
			...
			if (PROXY_STATUS.equalsIgnoreCase("on")) {
				...
				outName = HTTP + servlet.getServerName() + ":" + servlet.getServerPort() + context.getContextPath() + "/"
	...
	@Consumes({ MediaType.WILDCARD, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.TEXT_PLAIN })
	public String getQueries(@PathParam("host") String host, @PathParam("endpoint") String endpoint,
	...
	@Consumes({ MediaType.WILDCARD, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.TEXT_PLAIN })
	public String getExploreClasses(@PathParam("host") String host, @PathParam("endpoint") String endpoint,
			@PathParam("port") int port) throws EndpointCommunicationException {
		Vector<String> results = new Vector<String>();
		String format = "";
	...
	@Consumes({ MediaType.TEXT_PLAIN, MediaType.WILDCARD, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.TEXT_PLAIN })
	public String getExploreProperties(String classURI, @PathParam("host") String host,
			@PathParam("endpoint") String endpoint, @PathParam("port") int port) throws EndpointCommunicationException {
		Vector<String> results = new Vector<String>();
	...
	public class MyMap {
		...
		public MyMap(String url, String title, String creator, String license, String theme, String description,
				Extent extent, Vector<Distribution> ds) {
			this.id = url;
			this.type = "Dataset";
			this.title = title;
	...
	public Response requestMaps(@QueryParam("title") String title, @QueryParam("creator") String creator,
		...
		String mapURL = HTTP + servlet.getServerName() + ":" + servlet.getServerPort() + context.getContextPath()
				+ "?mapid=";
		...
		if (extent != null) {
			...
			query = query.concat(
					"FILTER (strdf:mbbIntersects(?geom, \"" + wkt + "\"^^<http://www.opengis.net/ont/geosparql#wktLiteral>)) . ");
		}
		query = query.concat("}");
		...
		for (int i = 0; i < results.size(); i = i + 7) {
			...
			ds.add(new Distribution("Distribution", "Sextant",
					mapURL.concat(results.get(i).substring(results.get(i).lastIndexOf("/") + 1))));

			MyMap tempMap = new MyMap(
					mapURL.concat(results.get(i).substring(results.get(i).lastIndexOf("/") + 1, results.get(i).length())),
					results.get(i + 1), results.get(i + 2), results.get(i + 3), results.get(i + 4), results.get(i + 5),
					new Extent("Location",
							results.get(i + 6).substring(results.get(i + 6).indexOf("POLYGON"), results.get(i + 6).length())),
					ds);
		...
		String output = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(new Maps(mapResults,
		...
		return Response.ok().entity(output).header("Access-Control-Allow-Origin", "*")
	...
	// }
	// if (location != null) {
	// double lat = Double.parseDouble(location.substring(0,
	// location.indexOf(",")));
	// double lon = Double.parseDouble(location.substring(location.indexOf(",")+1,
	// location.length()));
	...
	// "+Long.toString(status.getUser().getId())+" *** "+status.isRetweet()+" ***
	// "+status.isRetweeted());
	...
	// TwitterResults resultsObj = new TwitterResults(results.toString(),
	// higherStatusId, lowerStatusId);
	// ObjectMapper mapper = new ObjectMapper();
	// try {
	// output =
	// mapper.writerWithDefaultPrettyPrinter().writeValueAsString(resultsObj);
	...
	@Consumes({ MediaType.WILDCARD, MediaType.APPLICATION_JSON })
	@Produces({ MediaType.TEXT_PLAIN })
	public String mapSearchQuery(String inputData, @PathParam("host") String host, @PathParam("endpoint") String endpoint,
			@PathParam("port") int port) throws EndpointCommunicationException {
		String format = "";
		...
		if (!extent.equalsIgnoreCase("none")) {
			query = query.concat("FILTER (strdf:mbbIntersects(?geom, \"" + extent
	...
	public String parseImageGDAL(String fileUrl) {
		...
		try {
			Process p1 = Runtime.getRuntime()
					.exec(new String[] { GDAL_PATH + "gdalwarp", "-t_srs", "EPSG:4326", realFileUrl, fileTMP });
			try {
				p1.waitFor();
	...
	public String passQueryOS(String query, String fileName) throws IOException, QueryResultParseException,
		...
		try {
			...
			if (entity != null) {
				...
				FileOutputStream outputStream = new FileOutputStream(
						new File(context.getRealPath("/") + TMP_FOLDER + fileName + KML_FILE_EXT));
				...
				if (PROXY_STATUS.equalsIgnoreCase("on")) {
					fileURI = HTTP + PROXY_NAME + ":" + PROXY_PORT + context.getContextPath() + "/" + TMP_FOLDER + fileName
							+ KML_FILE_EXT;
				} else {
					fileURI = HTTP + servlet.getServerName() + ":" + servlet.getServerPort() + context.getContextPath() + "/"
							+ TMP_FOLDER + fileName + KML_FILE_EXT;
				}
			}
		...
		try {
			...
			if (response.getStatusCode() != 200) {
				...
				throw new RuntimeException(
						"Failed : HTTP error code : " + response.getStatusCode() + " " + response.getStatusText());
			}
	...
	private boolean hasSpatialResult(String response) {
		...
		try {
			...
			logger.error("[{}] Error while checking response for containment of spatial results: {}", moduleName,
					e.getMessage());
		}
	...
	public Vector<Layer> parseLayersInfo(String info) {
		...
		for (int i = 1; i < layerInfo.length; i++) {
			...
			Layer temp = new Layer(values.get(0), values.get(1), Boolean.parseBoolean(values.get(2)), values.get(3),
					values.get(4), values.get(5), values.get(6), values.get(7), values.get(8), values.get(9), values.get(10),
					values.get(11), values.get(12));
